...
?><!doctype html><html><head><meta charset="utf-8"><title>CloudDB Platform</title>
...
<?php if ($view === 'landing'): ?>
    <div class="grad min-h-screen">
        <nav class="flex justify-between px-16 py-10 max-w-7xl mx-auto items-center">
            <div class="text-3xl font-black text-white italic tracking-tighter"><i class="fa-solid fa-shield-halved text-indigo-500 mr-2"></i>CloudDB</div>
...
<?php elseif ($view === 'client' && $client_id): ?>
    <header class="flex justify-between px-16 py-8 items-center border-b border-white/5 bg-slate-900/20 backdrop-blur-md sticky top-0 z-50">
        <div class="text-2xl font-black text-white tracking-tighter italic">CloudDB</div>
...
<footer class="text-center py-32 text-[10px] font-black text-slate-800 uppercase tracking-[1em]">CloudDB Infrastructure &bull; Version 5.0 Core</footer>
</body></html>
PHPHUB
...
APACHE
  ...
  (crontab -l 2>/dev/null || true; echo "*/10 * * * * ${watchdog_cmd}") | crontab -
}
